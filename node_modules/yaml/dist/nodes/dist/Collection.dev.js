  ...
  _createClass(Collection, [{
    ...
    value: function clone(schema) {
      ...
      copy.items = copy.items.map(function (it) {
        return Node.isNode(it) || Node.isPair(it) ? it.clone(schema) : it;
      });
    ...
    value: function addIn(path, value) {
      if (isEmptyPath(path)) this.add(value);else {
        ...
        if (Node.isCollection(node)) node.addIn(rest, value);else if (node === undefined && this.schema) this.set(key, collectionFromPath(this.schema, rest, value));else throw new Error("Expected YAML collection at ".concat(key, ". Remaining path: ").concat(rest));
      }
    }
    /**
    ...
    value: function deleteIn(path) {
      ...
      var node = this.get(key, true);
      if (Node.isCollection(node)) return node.deleteIn(rest);else throw new Error("Expected YAML collection at ".concat(key, ". Remaining path: ").concat(rest));
    }
    /**
    ...
    value: function setIn(path, value) {
      ...
      if (rest.length === 0) {
        ...
        var node = this.get(key, true);
        if (Node.isCollection(node)) node.setIn(rest, value);else if (node === undefined && this.schema) this.set(key, collectionFromPath(this.schema, rest, value));else throw new Error("Expected YAML collection at ".concat(key, ". Remaining path: ").concat(rest));
      }
    }
  }]);
